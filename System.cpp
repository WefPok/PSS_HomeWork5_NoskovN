...
vector<Order> System::readActiveOrders() {
...
void System::writeActiveOrders(vector<Order> activeOrders) {
...
void System::validationPassengers() {
...
void System::validationDrivers() {
    ...
    for (int i = 0; i < numberOfUsers; i++) {
        ...
        for(int j=0;j<carsNumber;j++){
            ...
            Car car(model, number, color, type);
...
Passenger System::logInPassenger(Passenger passenger) {
    validationPassengers();
...
Driver System::logInDriver(Driver passenger) {
...
Admin System::logInAdmin(Admin passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
...
bool System::logInPassengerb(Passenger passenger) {
    // validationPassengers();
    for (int i = 0; i < validPassengers.size(); i++) {
        if (passenger.login == validPassengers[i].login && passenger.password == validPassengers[i].password)
            return 1;
...
bool System::logInDriverb(Driver passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
        if (passenger.login == validDrivers[i].login && passenger.password == validDrivers[i].password)
            return 1;
    }
...
bool System::logInAdminb(Admin passenger) {
...
int System::logInPassengeri(Passenger passenger) {
    validationPassengers();
    for (int i = 0; i < validPassengers.size(); i++) {
        if (passenger.login == validPassengers[i].login && passenger.password == validPassengers[i].password)
            return i;
...
int System::logInDriveri(Driver passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
        if (passenger.login == validDrivers[i].login && passenger.password == validDrivers[i].password)
            return i;
    }
...
int System::logInAdmini(Admin passenger) {
    for (int i = 0; i < validAdmins.size(); i++) {
        if (passenger.login == validAdmins[i].login && passenger.password == validAdmins[i].password)
            return i;
    }
...
void System::acceptRideByPassenger(Passenger passenger) {
    validationPassengers();
...
void System::createOrder(Passenger passenger, string from, string to, int carType) {
    validationPassengers();
    if(logInPassenger(passenger).isBlocked)
        throw blockError("Passenger blocked");
...
void System::acceptRideByDriver(Driver driver, int numberForAccept) {
    validationDrivers();
...
void System::endOrder(Driver driver) {
    validationPassengers();
    int k = logInDriveri(driver);
...
void System::lookForOrders(Driver driver) {
    validationDrivers();
    if(logInDriver(driver).isBlocked)
        throw blockError("Driver blocked");
    activeOrders = readActiveOrders();
...
vector<string> System::lookForPayments(Passenger passenger){
    validationPassengers();
    Passenger passenger1= logInPassenger(passenger);
    return passenger1.paymentMethods;
}

void System::addPayment(Passenger passenger,string method){
    validationPassengers();
...
vector<string> System::lookForPinnedAddresses(Passenger passenger){
    validationPassengers();
    Passenger passenger1= logInPassenger(passenger);
    return passenger1.pinnedAdresses;
}

void System::addPinnedAddress(Passenger passenger,string method){
    validationPassengers();
    int k= logInPassengeri(passenger);
...
void System::writePassengersFiles(vector<Passenger> passengers) {
...
void System::writeDriversFiles(vector<Driver> passengers) {
...
void System::writeAdminFiles(vector<Admin> admins){
...
void System::blockPassenger(Admin admin, int id){
    validationPassengers();
...
void System::unBlockPassenger(Admin admin, int id){
    validationPassengers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validPassengers.size();i++){
...
void System::blockDriver(Admin admin, int id){
    validationDrivers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validDrivers.size();i++){
...
void System::unBlockDriver(Admin admin, int id){
    validationDrivers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validDrivers.size();i++){
...
vector<Passenger> System::seePassengers(Admin admin){
    validationPassengers();
    if(!logInAdminb(admin)){
        vector<Passenger> p;
        return p;}
...
vector<Driver> System::seeDrivers(Admin admin){
    validationDrivers();
    if(!logInAdminb(admin)){
        vector<Driver> p;
        return p;}
...
vector<Order> System::seeActiveOrders(Admin admin){
    if(!logInAdminb(admin)){
        vector<Order> p;
        return p;}
...
void System::addCarDriver(Driver driver,Car car){
    readCarQueue();
...
void System::readCarQueue(){
    ...
    for(int i=0;i<n;i++){
        ...
        Car c(model,number,color,carType);
...
vector<string> System::seeCarsInQueue(Admin admin){
    vector<string> q;
    if(!logInAdminb(admin))
        return q;
...
void System::addCarAdmin(Admin admin,int n){
    if(!logInAdminb(admin))
        return;
    readCarQueue();
    for(int i=0;i<validDrivers.size();i++){
